...
?>
    ...
    <body>
        <div class="denied-container">
            ...
            <p class="denied-message">You don’t have permission to access this.</p>
...
// Fetch admins
$admins = $conn->query("SELECT id, name, phone, role FROM admin ORDER BY role, name");
        ...
        <div class="card">
            ...
            <div class="card-body" id="activityLog">
                <?php while ($log = $activity_logs->fetch_assoc()) { ?>
                    <div class="log-item">
                        <strong><?= $log['name'] ?></strong> - <?= $log['action'] ?><br>
                        <small><?= $log['message'] ?> | <?= $log['created_at'] ?></small>
                    </div>
                <?php } ?>
            </div>
        </div>
            ...
            <div class="card-body" id="loginLog">
                <?php if ($login_logs->num_rows > 0) {
                    while ($log = $login_logs->fetch_assoc()) { ?>
                        <div class="log-item">
                            <small><?= $log['message'] ?> | <?= $log['login_time'] ?></small>
                        </div>
                    <?php }
                } else { ?>
