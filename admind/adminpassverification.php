...
// Check if email is set in session or query string
if (!isset($_SESSION['email']) || !isset($_GET['email']) || $_SESSION['email'] !== $_GET['email']) {
    header('Location: adforgot.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result->num_rows === 1) {
        ...
        if ((string)$enteredOtp === (string)$storedOtp) {
            // OTP matches, proceed with password reset
            if ($newPassword === $confirmPassword) {
                // Hash the new password
                $hashedPassword = password_hash($newPassword, PASSWORD_DEFAULT);

                // Update the password and clear the OTP
                $updateSql = "UPDATE admin SET password = ?, otp = NULL WHERE email = ?";
...
<body>
    <div class="container">
        ...
        <p class="subtitle">Enter the 6-digit OTP sent to <strong><?php echo htmlspecialchars($userEmail); ?></strong> and set your new password.</p>
